...
const NftCardOpenContent: React.FC<INftCardContentProps> = ({ label, cost, icon }) => {
  return (
    <div className="flex-column items-center justify-center">
      <img
        className="rounded-lg xs:h-[139px] xs:w-[139px] md:max-2xl:h-56 md:max-2xl:w-56 2xl:h-56 2xl:w-56"
        src={icon}
        alt="NFT icon"
      />
      <div className="flex flex-row items-center justify-between xs:py-3 md:max-2xl:py-4 2xl:py-4">
        ...
        <div className="flex flex-row items-center justify-end gap-1 text-xs">
          <img
            className="xs:h-3 xs:w-3 md:max-2xl:h-4 md:max-2xl:w-4 2xl:h-4 2xl:w-4"
            src={dolarFrame}
            alt="Dolar svg"
          />
          <span className="font-bold text-[#F03033] xs:text-[8px] xs:leading-[10px] md:max-2xl:text-base 2xl:text-base">{`${cost} Coins`}</span>
        </div>
      </div>
      <OpenButton className=" xs:h-[33px] xs:w-[140px] md:max-2xl:h-[40px] md:max-2xl:w-[224px] 2xl:h-[40px] 2xl:w-[224px]" />
    </div>
  );
};
